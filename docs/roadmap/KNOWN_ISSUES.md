...
| CP-003 | tvUI suite shows 12 skipped tests (TV-8.2 Alert Markers) due to disabled describe block using wrong preview port and missing gotoChartsPro helper | Run `npm run test:tvui`; alert markers suite is `test.describe.skip` with conditional `test.skip()` when no alerts | **IN PROGRESS** (fix scheduled in current sprint: swap to gotoChartsPro + remove describe.skip) |
