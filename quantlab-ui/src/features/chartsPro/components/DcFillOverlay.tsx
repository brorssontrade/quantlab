 ...
 * DcFillOverlay
 * 
 * Canvas overlay that draws the background fill between upper and lower Donchian Channels.
 * 
...
interface DcFillOverlayProps {
  ...
  /** DC indicator result */
  dcResult: IndicatorWorkerResponse | null;
  /** Whether the fill is enabled */
  enabled?: boolean;
}

/**
 * DcFillOverlay Component
 * 
 * Renders the background fill between upper and lower Donchian Channels.
 */
export const DcFillOverlay = memo(function DcFillOverlay({
  chartRef,
  seriesRef,
  containerRef,
  dcResult,
  ...
  const shouldRender = enabled && dcResult !== null && dcResult._dcFill !== undefined;
  ...
  const draw = useCallback(() => {
    ...
    try {
      ...
      if (!canvas || !chart || !series || !container || !dcResult || !dcResult._dcFill || !enabled) {
      ...
      const { upper, lower, backgroundColor } = dcResult._dcFill;
  ...
  }, [chartRef, seriesRef, containerRef, dcResult, enabled]);
  ...
  return (
    <canvas
      ...
      data-testid="dc-fill-overlay"
    />
  );
});

export default DcFillOverlay;
