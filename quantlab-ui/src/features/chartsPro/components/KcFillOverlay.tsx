 ...
 * KcFillOverlay
 * 
 * Canvas overlay that draws the background fill between upper and lower Keltner Channels.
 * 
 ...
 * - Default color: very light blue rgba(33, 150, 243, 0.05) (~5% opacity)
 */
...
interface KcFillOverlayProps {
  ...
  /** KC indicator result */
  kcResult: IndicatorWorkerResponse | null;
  /** Whether the fill is enabled */
  enabled?: boolean;
}

/**
 * KcFillOverlay Component
 * 
 * Renders the background fill between upper and lower Keltner Channels.
 */
export const KcFillOverlay = memo(function KcFillOverlay({
  chartRef,
  seriesRef,
  containerRef,
  kcResult,
  ...
  const shouldRender = enabled && kcResult !== null && kcResult._kcFill !== undefined;
  ...
  const draw = useCallback(() => {
    ...
    try {
      ...
      if (!canvas || !chart || !series || !container || !kcResult || !kcResult._kcFill || !enabled) {
      ...
      // Get upper and lower channel data
      const { upper, lower, backgroundColor } = kcResult._kcFill;
      ...
      for (const segment of segments) {
        ...
        // Draw along upper channel
        for (let i = 1; i < segment.points.length; i++) {
          ctx.lineTo(segment.points[i].x, segment.points[i].yUpper);
        }
        
        // Then back along lower channel (in reverse, right to left)
  ...
  }, [chartRef, seriesRef, containerRef, kcResult, enabled]);
  ...
  return (
    <canvas
      ...
      data-testid="kc-fill-overlay"
    />
  );
});

export default KcFillOverlay;
