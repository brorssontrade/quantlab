 ...
 * 
 * Canvas overlay that draws the background fill between upper and lower Bollinger Bands.
 * 
...
interface BollingerBandsFillOverlayProps {
  ...
  /** BB indicator result */
  bbResult: IndicatorWorkerResponse | null;
  /** Whether the fill is enabled */
  enabled?: boolean;
}

/**
 * BollingerBandsFillOverlay Component
 * 
 * Renders the background fill between upper and lower Bollinger Bands.
 */
export const BollingerBandsFillOverlay = memo(function BollingerBandsFillOverlay({
  chartRef,
  seriesRef,
  containerRef,
  bbResult,
  enabled = true,
}: BollingerBandsFillOverlayProps) {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const rafRef = useRef<number | null>(null);
  const isDrawingRef = useRef(false);

  // Check if we should render
  const shouldRender = enabled && bbResult !== null && bbResult._bbData !== undefined;
  ...
  const draw = useCallback(() => {
    ...
    try {
      ...
      if (!canvas || !chart || !series || !container || !bbResult || !bbResult._bbData || !enabled) {
      ...
      const { upper, lower, backgroundColor } = bbResult._bbData;
  ...
  }, [chartRef, seriesRef, containerRef, bbResult, enabled]);
  ...
  return (
    <canvas
      ...
      data-testid="bb-fill-overlay"
    />
  );
});

export default BollingerBandsFillOverlay;
