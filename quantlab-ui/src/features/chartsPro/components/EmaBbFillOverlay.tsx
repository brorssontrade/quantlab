 ...
 * EmaBbFillOverlay
 * 
 * Canvas overlay that draws the background fill between upper and lower EMA Bollinger Bands.
 * Used when EMA smoothingType = "sma_bb".
 * 
 * Why canvas overlay instead of LWC area series:
 * - Fill needs to be between two dynamic lines (upper/lower BB on EMA)
...
interface EmaBbFillOverlayProps {
  ...
  emaResult: IndicatorWorkerResponse | null;
  /** Whether the fill is enabled */
  enabled?: boolean;
}

/**
 * EmaBbFillOverlay Component
 * 
 * Renders the background fill between upper and lower EMA Bollinger Bands.
 */
export const EmaBbFillOverlay = memo(function EmaBbFillOverlay({
  chartRef,
  seriesRef,
  containerRef,
  emaResult,
  enabled = true,
}: EmaBbFillOverlayProps) {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const rafRef = useRef<number | null>(null);
  const isDrawingRef = useRef(false);

  // Check if we should render
  const shouldRender = enabled && emaResult !== null && emaResult._emaFill !== undefined && emaResult._emaFill.showBBFill;
  ...
  const draw = useCallback(() => {
    ...
    try {
      ...
      if (!canvas || !chart || !series || !container || !emaResult || !emaResult._emaFill || !enabled) {
      ...
      const { upper, lower, bbFillColor } = emaResult._emaFill;
  ...
  }, [chartRef, seriesRef, containerRef, emaResult, enabled]);
  ...
  return (
    <canvas
      ...
      data-testid="ema-bb-fill-overlay"
    />
  );
});

export default EmaBbFillOverlay;
